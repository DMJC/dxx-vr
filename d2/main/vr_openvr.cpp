...
static void vr_openvr_dump_menu_texture(void)
{
	...
	const char *dump_dir = "screenshots";
	if (!PHYSFSX_exists(dump_dir, 0))
	{
		if (!PHYSFS_mkdir(dump_dir))
			con_printf(CON_URGENT, "PHYSFS: could not create '%s': %s\n", dump_dir, PHYSFS_getLastError());
	}
	...
	if (!write_png(filename, &pdata))
	{
		...
		con_printf(CON_URGENT, "Could not create VR menu PNG '%s' (write dir: %s): %s\n",
			filename, write_dir ? write_dir : "(null)", PHYSFS_getLastError());
	}
...
static void vr_openvr_apply_eye_modelview(void)
{
...
void vr_openvr_begin_frame(void)
{
	...
	if (poses[vr::k_unTrackedDeviceIndex_Hmd].bPoseIsValid)
	{
		...
		const float c = -1.0f;
		const float r00 = mat.m[0][0];
		const float r01 = mat.m[0][1];
		const float r02 = mat.m[0][2];
		const float r10 = mat.m[1][0];
		const float r11 = mat.m[1][1];
		const float r12 = mat.m[1][2];
		const float r20 = mat.m[2][0];
		const float r21 = mat.m[2][1];
		const float r22 = mat.m[2][2];

		vr_head_orient.rvec.x = fl2f(r00);
		vr_head_orient.rvec.y = fl2f(r10);
		vr_head_orient.rvec.z = fl2f(c * r20);
		vr_head_orient.uvec.x = fl2f(r01);
		vr_head_orient.uvec.y = fl2f(r11);
		vr_head_orient.uvec.z = fl2f(c * r21);
		vr_head_orient.fvec.x = fl2f(r02);
		vr_head_orient.fvec.y = fl2f(r12);
		vr_head_orient.fvec.z = fl2f(c * r22);

		vr_head_pos.x = fl2f(mat.m[0][3]);
		vr_head_pos.y = fl2f(mat.m[1][3]);
		vr_head_pos.z = fl2f(c * mat.m[2][3]);
		vr_has_pose = true;
	}
	else
	{
