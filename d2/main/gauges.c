...
static void gauge_push_font_scale(float *old_scale_x, float *old_scale_y)
{
...
{
	FNTScaleX = old_scale_x;
	FNTScaleY = old_scale_y;
}
...
void sb_show_score()
{
	...
	if ( (Game_mode & GM_MULTI) && !((Game_mode & GM_MULTI_COOP) || (Game_mode & GM_MULTI_ROBOTS)) )
#ifdef USE_OPENVR
		gr_printf(HUD_SCALE_X(SB_SCORE_LABEL_X) + offset_x, HUD_SCALE_Y(SB_SCORE_Y) + offset_y,"%s:", TXT_KILLS);
#else
		gr_printf(HUD_SCALE_X(SB_SCORE_LABEL_X),HUD_SCALE_Y(SB_SCORE_Y),"%s:", TXT_KILLS);
#endif
	else
#ifdef USE_OPENVR
		gr_printf(HUD_SCALE_X(SB_SCORE_LABEL_X) + offset_x,HUD_SCALE_Y(SB_SCORE_Y) + offset_y, "%s:", TXT_SCORE);
#else
	...
	draw_x = x + offset_x;
	draw_y = y + offset_y;
	draw_right = HUD_SCALE_X(SB_SCORE_RIGHT) + offset_x;
#else
	draw_x = x;
	draw_y = y;
	draw_right = HUD_SCALE_X(SB_SCORE_RIGHT);
#endif
...
#ifdef USE_OPENVR
		gr_printf(x+HUD_SCALE_X_AR(bm->bm_w) + offset_x, y + offset_y, " x %d", Players[pnum].secondary_ammo[PROXIMITY_INDEX]);
#else
...
#ifdef USE_OPENVR
		     gr_printf(FSPACX(1) + offset_x, (grd_curcanv->cv_bitmap.bm_h-LINE_SPACING) + offset_y,"%s: %i", TXT_ENERGY, f2ir(Players[pnum].energy));
#else
...
#ifdef USE_OPENVR
	gr_printf(FSPACX(1) + offset_x, grd_curcanv->cv_bitmap.bm_h+y + offset_y, "burn: %d%%" , fixmul(Players[pnum].afterburner_charge,100));
#else
...
#ifdef USE_OPENVR				
				gr_printf(x + offset_x,y-(LINE_SPACING*1) + offset_y,"V:%i",f2i((unsigned int)Players[pnum].primary_ammo[1] * VULCAN_AMMO_SCALE));
#else
...
#ifdef USE_OPENVR				
				gr_printf(x+FSPACX(9) + offset_x,y-(LINE_SPACING*2)+offset_y,"G:%i",f2i((unsigned int)Players[pnum].primary_ammo[1] * VULCAN_AMMO_SCALE));
#else
...
void hud_show_weapons(void)
{
	...
	else
	{
...
#ifdef USE_OPENVR
		gr_string(grd_curcanv->cv_bitmap.bm_w-w-FSPACX(1) + offset_x, y-(LINE_SPACING*2) + offset_y, disp_primary_weapon_name);
#else
		...
		gr_get_string_size(weapon_str, &w, &h, &aw );
#ifdef USE_OPENVR
		gr_string(grd_curcanv->cv_bitmap.bm_w-w-FSPACX(1) + offset_x, y-LINE_SPACING + offset_y, weapon_str);
		show_bomb_count(grd_curcanv->cv_bitmap.bm_w-FSPACX(1) + offset_x, y-(LINE_SPACING*3) + offset_y,-1,1,1);
#else
...
void hud_show_cloak_invuln(void)
{
	...
	if (Players[pnum].flags & PLAYER_FLAGS_CLOAKED) {
		...
		{
#ifdef USE_OPENVR
			gr_printf(FSPACX(1) + offset_x, y + offset_y, "%s", TXT_CLOAKED);
#else
	...
	if (Players[pnum].invulnerable_time + INVULNERABLE_TIME_MAX - GameTime64 > 0) {
		if (Players[pnum].flags & PLAYER_FLAGS_INVULNERABLE) {
			...
			{
#ifdef USE_OPENVR
				gr_printf(FSPACX(1) + offset_x, y + offset_y, "%s", TXT_INVULNERABLE);
#else
...
void hud_show_shield(void)
{
	...
	if (PlayerCfg.HudMode<2) {
		...
		if ( Players[pnum].shields >= 0 )	{
			if (Game_mode & GM_MULTI)
#ifdef USE_OPENVR
   			     gr_printf(FSPACX(1) + offset_x, (grd_curcanv->cv_bitmap.bm_h-(LINE_SPACING*6)) + offset_y,"%s: %i", TXT_SHIELD, f2ir(Players[pnum].shields));
#else
			     gr_printf(FSPACX(1), (grd_curcanv->cv_bitmap.bm_h-(LINE_SPACING*6)),"%s: %i", TXT_SHIELD, f2ir(Players[pnum].shields));
#endif
			else
#ifdef USE_OPENVR
			     gr_printf(FSPACX(1) + offset_x, (grd_curcanv->cv_bitmap.bm_h-(LINE_SPACING*2)) + offset_y,"%s: %i", TXT_SHIELD, f2ir(Players[pnum].shields));
#else
			...
			if (Game_mode & GM_MULTI)
#ifdef USE_OPENVR
			     gr_printf(FSPACX(1) + offset_x, (grd_curcanv->cv_bitmap.bm_h-(LINE_SPACING*6)) + offset_y,"%s: 0", TXT_SHIELD );
#else
			     gr_printf(FSPACX(1), (grd_curcanv->cv_bitmap.bm_h-(LINE_SPACING*6)),"%s: 0", TXT_SHIELD );
#endif
			else
#ifdef USE_OPENVR
			     gr_printf(FSPACX(1) + offset_x, (grd_curcanv->cv_bitmap.bm_h-(LINE_SPACING*2)) + offset_y,"%s: 0", TXT_SHIELD );
#else
...
void sb_show_lives()
{
	...
	if (Game_mode & GM_MULTI)
#ifdef USE_OPENVR
		gr_printf(HUD_SCALE_X(SB_LIVES_LABEL_X) + offset_x,HUD_SCALE_Y(y) + offset_y,"%s:", TXT_DEATHS);
#else
		gr_printf(HUD_SCALE_X(SB_LIVES_LABEL_X),HUD_SCALE_Y(y),"%s:", TXT_DEATHS);
#endif
	else
#ifdef USE_OPENVR
		gr_printf(HUD_SCALE_X(SB_LIVES_LABEL_X) + offset_x,HUD_SCALE_Y(y) + offset_y,"%s:", TXT_LIVES);
#else
	...
	if (Game_mode & GM_MULTI)
	{
		...
		x = HUD_SCALE_X(SB_SCORE_RIGHT)-w-FSPACX(1);
#ifdef USE_OPENVR
		gr_string(x + offset_x, HUD_SCALE_Y(y) + offset_y, killed_str);
#else
...
void show_time()
{
	...
	gr_set_fontcolor(Color_0_31_0, -1 );
#ifdef USE_OPENVR
	gr_printf(SWIDTH-FSPACX(30) + offset_x,GHEIGHT-(LINE_SPACING*11) + offset_y,"%d:%02d", mins, secs);
#else
...
void draw_wbu_overlay()
{
	...
	bm = &GameBitmaps[cockpit_bitmap[cockpit_idx].index];
...
#ifdef USE_OPENVR
	hud_bitblt (HUD_SCALE_X(LEFT_ENERGY_GAUGE_X) + offset_x, HUD_SCALE_Y(LEFT_ENERGY_GAUGE_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_ENERGY_LEFT) ]);
#else
...
#ifdef USE_OPENVR
	hud_bitblt (HUD_SCALE_X(RIGHT_ENERGY_GAUGE_X) + offset_x, HUD_SCALE_Y(RIGHT_ENERGY_GAUGE_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_ENERGY_RIGHT) ]);
#else
	...
	if (energy < 100)
		for (y=0; y < HUD_SCALE_Y(RIGHT_ENERGY_GAUGE_H); y++) {
			...
			if (x1 < (y*aplitscale)/3)
				x1 = (y*aplitscale)/3;
#ifdef USE_OPENVR
			if (x2 > x1) gr_uline( i2f(x1+HUD_SCALE_X(RIGHT_ENERGY_GAUGE_X)+offset_x), i2f(y+HUD_SCALE_Y(RIGHT_ENERGY_GAUGE_Y)+offset_y), i2f(x2+HUD_SCALE_X(RIGHT_ENERGY_GAUGE_X)+offset_x), i2f(y+HUD_SCALE_Y(RIGHT_ENERGY_GAUGE_Y)+offset_y) );
#else
...
void draw_afterburner_bar(int afterburner)
{
...
#ifdef USE_OPENVR
	hud_bitblt( HUD_SCALE_X(AFTERBURNER_GAUGE_X) + offset_x, HUD_SCALE_Y(AFTERBURNER_GAUGE_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_AFTERBURNER) ]);
#else
	...
	for (y = 0; y < not_afterburner; y++) {
		for (i = HUD_SCALE_Y (y), j = HUD_SCALE_Y (y + 1); i < j; i++) {
			gr_rect (
#ifdef USE_OPENVR
				HUD_SCALE_X (AFTERBURNER_GAUGE_X + pabt [y * 2]) + offset_x,
				HUD_SCALE_Y (AFTERBURNER_GAUGE_Y-1) + i + offset_y,
				HUD_SCALE_X (AFTERBURNER_GAUGE_X + pabt [y * 2 + 1] + 1) + offset_x,
				HUD_SCALE_Y (AFTERBURNER_GAUGE_Y) + i + offset_y);
#else
...
#ifdef USE_OPENVR
	hud_bitblt( HUD_SCALE_X(SHIELD_GAUGE_X) + offset_x, HUD_SCALE_Y(SHIELD_GAUGE_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_SHIELDS+9-bm_num) ]);
#else
...
void draw_player_ship(int cloak_state,int x, int y)
{
	...
	gr_set_current_canvas(NULL);
#ifdef USE_OPENVR
	hud_bitblt( HUD_SCALE_X(x) + offset_x, HUD_SCALE_Y(y) + offset_y, bm);
#else
	hud_bitblt( HUD_SCALE_X(x), HUD_SCALE_Y(y), bm);
#endif
	gr_settransblend(cloak_fade_value, GR_BLEND_NORMAL);
#ifdef USE_OPENVR
	gr_rect(HUD_SCALE_X(x-3) + offset_x, HUD_SCALE_Y(y-3) + offset_y, HUD_SCALE_X(x+bm->bm_w+3) + offset_x, HUD_SCALE_Y(y+bm->bm_h+3) + offset_y);
#else
...
#ifdef USE_OPENVR
		hud_bitblt( HUD_SCALE_X(GAUGE_BLUE_KEY_X) + offset_x, HUD_SCALE_Y(GAUGE_BLUE_KEY_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_BLUE_KEY) ]);
#else
...
#ifdef USE_OPENVR
		hud_bitblt( HUD_SCALE_X(GAUGE_BLUE_KEY_X) +offset_x, HUD_SCALE_Y(GAUGE_BLUE_KEY_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_BLUE_KEY_OFF) ]);
#else
...
#ifdef USE_OPENVR
		hud_bitblt( HUD_SCALE_X(GAUGE_GOLD_KEY_X) + offset_x, HUD_SCALE_Y(GAUGE_GOLD_KEY_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_GOLD_KEY) ]);
#else
...
#ifdef USE_OPENVR
		hud_bitblt( HUD_SCALE_X(GAUGE_GOLD_KEY_X) +offset_x, HUD_SCALE_Y(GAUGE_GOLD_KEY_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_GOLD_KEY_OFF) ]);
#else
...
#ifdef USE_OPENVR
		hud_bitblt( HUD_SCALE_X(GAUGE_RED_KEY_X) + offset_x, HUD_SCALE_Y(GAUGE_RED_KEY_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_RED_KEY) ]);
#else
...
#ifdef USE_OPENVR
		hud_bitblt( HUD_SCALE_X(GAUGE_RED_KEY_X) +offset_x, HUD_SCALE_Y(GAUGE_RED_KEY_Y) + offset_y, &GameBitmaps[ GET_GAUGE_INDEX(GAUGE_RED_KEY_OFF) ]);
#else
...
void draw_weapon_info_sub(int info_index,gauge_box *box,int pic_x,int pic_y,char *name,int text_x,int text_y)
{
	...
	gr_setcolor(BM_XRGB(0,0,0));
#ifdef USE_OPENVR
	gr_rect(HUD_SCALE_X(box->left) + offset_x, HUD_SCALE_Y(box->top) + offset_y, HUD_SCALE_X(box->right) + offset_x, HUD_SCALE_Y(box->bot) + offset_y);
#else
	...
	Assert(bm != NULL);
#ifdef USE_OPENVR
	hud_bitblt(HUD_SCALE_X(pic_x) + offset_x, HUD_SCALE_Y(pic_y) + offset_y, bm);
#else
...
#ifdef USE_OPENVR
		gr_string(text_x + offset_x, text_y + offset_y, name);
#else
...
void draw_ammo_info(int x,int y,int ammo_count,int primary)
{
	if (!PlayerCfg.HudMode)
	{
		...
		gr_set_fontcolor(BM_XRGB(20,0,0),-1 );
#ifdef USE_OPENVR
		gr_printf(x + offset_x,y + offset_y,"%03d",ammo_count);
#else
...
void sb_draw_shield_bar(int shield)
{
	...
	PAGE_IN_GAUGE( GAUGE_SHIELDS+9-bm_num );
#ifdef USE_OPENVR
	hud_bitblt( HUD_SCALE_X(SB_SHIELD_GAUGE_X) + offset_x, HUD_SCALE_Y(SB_SHIELD_GAUGE_Y) + offset_y, &GameBitmaps[GET_GAUGE_INDEX(GAUGE_SHIELDS+9-bm_num) ]);
#else
...
#ifdef USE_OPENVR
	hud_bitblt( HUD_SCALE_X(SB_GAUGE_KEYS_X) + offset_x, HUD_SCALE_Y(SB_GAUGE_BLUE_KEY_Y) + offset_y, bm);
#else	
...
#ifdef USE_OPENVR
	hud_bitblt( HUD_SCALE_X(SB_GAUGE_KEYS_X) + offset_x, HUD_SCALE_Y(SB_GAUGE_GOLD_KEY_Y) + offset_y, bm);
#else	
...
#ifdef USE_OPENVR
	hud_bitblt( HUD_SCALE_X(SB_GAUGE_KEYS_X) + offset_x, HUD_SCALE_Y(SB_GAUGE_RED_KEY_Y) + offset_y, bm);
#else	
...
void draw_invulnerable_ship()
{
	...
	if (Players[pnum].invulnerable_time+INVULNERABLE_TIME_MAX-GameTime64 > 0 && (Players[pnum].invulnerable_time + INVULNERABLE_TIME_MAX - GameTime64 > F1_0*4 || GameTime64 & 0x8000))
	{
		...
		if (PlayerCfg.CurrentCockpitMode == CM_STATUS_BAR)	{
...
#ifdef USE_OPENVR
			hud_bitblt( HUD_SCALE_X(SB_SHIELD_GAUGE_X) + offset_x, HUD_SCALE_Y(SB_SHIELD_GAUGE_Y) + offset_y, &GameBitmaps[GET_GAUGE_INDEX(GAUGE_INVULNERABLE+invulnerable_frame) ]);
#else
			...
			PAGE_IN_GAUGE( GAUGE_INVULNERABLE+invulnerable_frame );
#ifdef USE_OPENVR
			hud_bitblt( HUD_SCALE_X(SHIELD_GAUGE_X) + offset_x, HUD_SCALE_Y(SHIELD_GAUGE_Y) + offset_y, &GameBitmaps[GET_GAUGE_INDEX(GAUGE_INVULNERABLE+invulnerable_frame)]);
#else
...
void hud_show_kill_list()
{
	...
	for (i=0;i<players;i++) {
...
#ifdef USE_OPENVR
        gr_printf(x0 + offset_x,y + offset_y,"%s",name);		
#else
		...
		if (Show_kill_list==2)
		{
...
#ifdef USE_OPENVR
				gr_printf (x1 + offset_x,y + offset_y,"%d%%",(int)((float)((float)Players[player_num].net_kills_total/((float)Players[player_num].net_killed_total+(float)Players[player_num].net_kills_total))*100.0));
#else
...
#ifdef USE_OPENVR
				gr_printf(x1 + offset_x,y + offset_y,"%3d(%d)",team_kills[i],Netgame.TeamKillGoalCount[i]);
#else
...
#ifdef USE_OPENVR
				gr_printf(x1 + offset_x,y + offset_y,"%3d",team_kills[i]);
#else
...
#ifdef USE_OPENVR
			gr_printf(x1 + offset_x,y + offset_y,"%-6d",Players[player_num].score);
#else
			...
			if (Netgame.PlayTimeAllowed || Netgame.KillGoal)
#ifdef USE_OPENVR
				gr_printf(x1 + offset_x,y + offset_y,"%3d(%d)",Players[player_num].net_kills_total,Players[player_num].KillGoalCount);
#else
				gr_printf(x1,y,"%3d(%d)",Players[player_num].net_kills_total,Players[player_num].KillGoalCount);
#endif
			else
#ifdef USE_OPENVR
				gr_printf(x1 + offset_x,y + offset_y,"%3d",Players[player_num].net_kills_total);
#else
		...
		if((Show_network_stats && !is_observer()) && player_num != Player_num && Players[player_num].connected && Show_kill_list != 3) {
...
#ifdef USE_OPENVR
				gr_printf(lagx + offset_x,y + offset_y,"% 3d", lag);
#else
...
#ifdef USE_OPENVR
				gr_printf(loss_upx + offset_x,y + offset_y,"% 3d%%", loss_up);
#else
			...
			if(loss_up > 0 || loss_down > 0) {
#ifdef USE_OPENVR
				gr_printf(loss_downx + offset_x,y + offset_y,"% 3d%%", loss_down);
#else
			...
			if(connection_statuses[player_num].type == CONNT_PROXY) {
				...
				if(loss_up > 0 || loss_down > 0) {
#ifdef USE_OPENVR
				gr_printf(cnxx + FSPACX(5) + offset_x, y + offset_y,"P");
#else
				gr_printf(cnxx + FSPACX(5), y,"P");
#endif
				} else {
#ifdef USE_OPENVR
				gr_printf(loss_upx + offset_x,y + offset_y,"P");
#else
...
int observer_draw_player_card(int pnum, int color, int x, int y) {
...
#ifdef USE_OPENVR
	gr_printf((x + OBS_PLAYER_CARD_WIDTH / 2 - sw / 2) + offset_x, y + offset_y, "%s", Players[pnum].callsign);
#else
	...
	if ((Game_mode & GM_MULTI_COOP) || (Game_mode & GM_MULTI_ROBOTS)) {
...
#ifdef USE_OPENVR
		gr_printf((x + OBS_PLAYER_CARD_WIDTH / 2 - sw / 2) + offset_x, y + offset_y, "%s", score);
#else
		...
		gr_get_string_size(score, &sw, &sh, &saw);
#ifdef USE_OPENVR
		gr_printf((x + OBS_PLAYER_CARD_WIDTH - sw - 3) + offset_x, y + offset_y, "%s", score);
#else
...
#ifdef USE_OPENVR
			gr_printf((x + 3) + offset_x, y + offset_y, "%s", shields);
#else
...
#ifdef USE_OPENVR
			gr_printf((x + OBS_PLAYER_CARD_WIDTH - 1 - sw) + offset_x, y + offset_y, primary_ammo);
#else
...
#ifdef USE_OPENVR
			gr_printf((x + OBS_PLAYER_CARD_WIDTH - 1 - sw) + offset_x, y + offset_y, secondary_ammo);
#else
...
int maybe_show_observers(int startY) {
...
#ifdef USE_OPENVR
	gr_printf(x + offset_x, y + offset_y, "Observers:");
#else
	...
	for (int i = 0; i < Netgame.max_numobservers; i++) {
		...
		x = grd_curcanv->cv_bitmap.bm_w - w - 5;
#ifdef USE_OPENVR
		gr_printf(x + offset_x, y + offset_y, "%s", Netgame.observers[i].callsign);
#else
